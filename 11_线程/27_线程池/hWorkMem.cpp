...
namespace hThread
{
				...
				_runCv.wait(lk, [&]
					{
						...
						{
							COUT_LK("Tsk_" << _pTask->getId() << " 不是重复型任务,由WorkMem_" << _id << "执行...");
							return true;
						}
						...
						if (_pTask->canRepeat())
						{
							COUT_LK("Tsk_" << _pTask->getId() << " 可重复执行,由WorkMem_" << _id << "执行...");
							return true;
						}

						COUT_LK("Tsk_" << _pTask->getId() << " 不可执行,释放WorkMem_" << _id << "...");
		...
		hMemBase(id)
	{
		...
		COUT_LK("WorkMem_" << _id << " 工作线程创建...");
	}

	hWorkMem::~hWorkMem()
	{
		COUT_LK("WorkMem_" << _id << " 工作线程释放...");
	}
